...
Script to automate cooling technology and water source mapping to NEMS unit dataset! 
@author: skhanal
"""
import numpy as np
import pandas as pd
...
ct_wst_append = ct_wst_pca_region_code_map[['index', 'ctt', 'wst']].copy()

subprocess.call(["python", "coolingtech_process_IO.py"], cwd="coolingtech") #os.system('python coolingtech/coolingtech_process_IO.py')
val_i=pd.read_csv('coolingtech/i_coolingtech_watersource.csv', skiprows = 0)
val_i_list = list(val_i['*i'])

nems_orig = pd.read_csv('inputs/2020nems_needs_ct.csv', index_col=None,  low_memory=False) # input processed NEMS database
nems_coolingwatertech = pd.merge(nems_orig, ct_wst_append[['index', 'ctt', 'wst']], on = 'index', how = 'left') 
nems_coolingwatertech.loc[(nems_coolingwatertech['Commercial.Online.Year.Quarter'] >= 2010) & (nems_coolingwatertech['wst'] == 'fsa'), 'wst'] = 'fsu'
nems_coolingwatertech.insert(1, 'coolingwatertech', nems_coolingwatertech['tech']+'_'+nems_coolingwatertech['ctt']+'_'+nems_coolingwatertech['wst'])
nems_coolingwatertech.loc[~nems_coolingwatertech['coolingwatertech'].isin(val_i_list), 'coolingwatertech'] =  nems_coolingwatertech['tech']
nems_coolingwatertech.index+=1
#nems_coolingwatertech.drop(['ctt', 'wst' ], axis=1, inplace=True)

nems_coolingwatertech.to_csv("ReEDS_generator_database_final_EIA-NEMS_input.csv")
